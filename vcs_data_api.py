...
class GitHub(VersionControlSystem):
    ...
    def is_private(self):
        ...
        try:
            ...
            return repo.private
        except GithubException.RateLimitExceededException as rate_limit_exception:
            reset_time = datetime.utcfromtimestamp(float(rate_limit_exception.headers.get('X-RateLimit-Reset')))
            time_to_wait = reset_time - datetime.utcnow()
            raise HTTPException(status_code=403, detail=f"You have reached your API request limit. Please wait for {time_to_wait} and try again.")
